angular.module('app').controller('LevelUpdateController', function($scope, $compile, $cookies, $filter, $state, $routeParams, HTTPService, IndexOverlayFactory) {
    ...
    $scope.loadData = function(AutoID){
        ...
        HTTPService.clientRequest('level/get', params).then(function(result){
            console.log(result);
    ...
    $scope.saveData = function(Data, SecretData){
        ...
        HTTPService.clientRequest('level/update', params).then(function(result){
            console.log(result);
            if(result.data.STATUS == 'OK'){
                if(checkEmptyField($scope.Data.AutoID)){
                    // window.location.href = '#/level/update/' + result.data.DATA.AutoID;
                }else{
