angular.module('app').controller('ChapterMainController', function($scope, $compile, $cookies, $filter, $state, $routeParams, HTTPService, IndexOverlayFactory) {
    ...
    $scope.loadList = function(SubjectCode, condition){
        IndexOverlayFactory.overlayShow();
        ...
        HTTPService.clientRequest('chapter/list/manage', params).then(function(result){
            console.log(result);
    ...
    $scope.search = function(SubjectCode, condition){
        $scope.loadList(SubjectCode, condition);
    }

    $scope.pageChanged = function() {
        $scope.loadList($scope.SubjectCode, $scope.condition);
    };
